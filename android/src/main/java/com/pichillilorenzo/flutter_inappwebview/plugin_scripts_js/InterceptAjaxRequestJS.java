...
public class InterceptAjaxRequestJS {
        ...
        public static final String FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_AJAX_REQUEST_JS_SOURCE = JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
        ...
        public static final String INTERCEPT_AJAX_REQUEST_JS_SOURCE = "(function(ajax) {" +
                        ...
                        "    var self = this;" +
                        "    var w = (window.top == null || window.top === window) ? window : window.top;" +
                        "    if (w." + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_AJAX_REQUEST_JS_SOURCE + " == null || w."
                        ...
                        "          }" +
                        "        };" +
                        "        window." + JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
                        + ".callHandler('onAjaxProgress', ajaxRequest).then(function(result) {" +
                        "          if (result != null) {" +
                        "            switch (result) {" +
                        "              case 0:" +
                        "                self.abort();" +
                        ...
                        "          var w = (window.top == null || window.top === window) ? window : window.top;" +
                        "          if (w." + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_AJAX_REQUEST_JS_SOURCE
                        + " == null || w." + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_AJAX_REQUEST_JS_SOURCE + " == true) {" +
                        "            var headers = this.getAllResponseHeaders();" +
                        "            var responseHeaders = {};" +
                        "            if (headers != null) {" +
                        "              var arr = headers.trim().split(/[\\r\\n]+/);" +
                        ...
                        "              };" +
                        "              window." + JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
                        + ".callHandler('onAjaxReadyStateChange', ajaxRequest).then(function(result) {" +
                        "                if (result != null) {" +
                        "                  switch (result) {" +
                        "                    case 0:" +
                        "                      self.abort();" +
                        ...
                        + ".convertBodyRequest(data).then(function(data) {" +
                        "        var ajaxRequest = {" +
                        "          data: data," +
                        "          method: self._flutter_inappwebview_method," +
                        "          url: self._flutter_inappwebview_url," +
                        ...
                        "        };" +
                        "        window." + JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
                        + ".callHandler('shouldInterceptAjaxRequest', ajaxRequest).then(function(result) {" +
                        "          if (result != null) {" +
                        "            switch (result) {" +
                        "              case 0:" +
                        "                self.abort();" +
                        "                return;" +
                        "            };" +
                        "            if (result.data != null && !" + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.data) && result.data.length > 0) {" +
                        "              var bodyString = " + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".arrayBufferToString(result.data);" +
                        "              if (" + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isBodyFormData(bodyString)) {" +
                        "                var formDataContentType = " + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".getFormDataContentType(bodyString);" +
                        ...
                        "            }" +
                        "            if (" + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.data) || result.data == null) {" +
                        "              data = result.data;" +
