...
public class InterceptFetchRequestJS {
        ...
        public static final String FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_FETCH_REQUEST_JS_SOURCE = JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
        ...
        public static final String INTERCEPT_FETCH_REQUEST_JS_SOURCE = "(function(fetch) {"
                        ...
                        + "    if (w." + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_FETCH_REQUEST_JS_SOURCE + " == null || w."
                        + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_FETCH_REQUEST_JS_SOURCE + " == true) {"
                        + "      var fetchRequest = {" + "        url: null," + "        method: null,"
                        + "        headers: null," + "        body: null," + "        mode: null,"
                        + "        credentials: null," + "        cache: null," + "        redirect: null,"
                        + "        referrer: null," + "        referrerPolicy: null," + "        integrity: null,"
                        + "        keepalive: null" + "      };" + "      if (resource instanceof Request) {"
                        + "        fetchRequest.url = resource.url;" + "        fetchRequest.method = resource.method;"
                        + "        fetchRequest.headers = resource.headers;"
                        + "        fetchRequest.body = resource.body;" + "        fetchRequest.mode = resource.mode;"
                        ...
                        + "        fetchRequest.keepalive = resource.keepalive;" + "      } else {"
                        + "        fetchRequest.url = resource != null ? resource.toString() : null;"
                        + "        if (init != null) {" + "          fetchRequest.method = init.method;"
                        + "          fetchRequest.headers = init.headers;" + "          fetchRequest.body = init.body;"
                        ...
                        + "      if (fetchRequest.headers instanceof Headers) {" + "        fetchRequest.headers = "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME + ".convertHeadersToJson(fetchRequest.headers);"
                        + "      }" + "      fetchRequest.credentials = " + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".convertCredentialsToJson(fetchRequest.credentials);" + "      return "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".convertBodyRequest(null).then(function(body) {/*rafuck*/"
                        + "        body = fetchRequest.body; fetchRequest.body = null; /*rafuck*/"
                        + "        return window." + JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
                        + ".callHandler('shouldInterceptFetchRequest', fetchRequest).then(function(result) {"
                        + "          if (result != null) {" + "            switch (result.action) {"
                        + "              case 0:" + "                var controller = new AbortController();"
                        + "                if (init != null) {" + "                  init.signal = controller.signal;"
                        + "                } else {" + "                  init = {"
                        + "                    signal: controller.signal" + "                  };" + "                }"
                        + "                controller.abort();" + "                break;" + "            }"
                        + "            if (result.body != null && !" + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.body) && result.body.length > 0) {" + "              var bodyString = "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME + ".arrayBufferToString(result.body);"
                        + "              if (" + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isBodyFormData(bodyString)) {" + "                var formDataContentType = "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME + ".getFormDataContentType(bodyString);"
                        ...
                        + "                }" + "              }" + "            }"
                        + "            resource = result.url;" + "            if (init == null) {"
                        + "              init = {};" + "            }"
                        ...
                        + "              init.headers = " + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".convertJsonToHeaders(result.headers);" + "            }" + "            if ("
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.body) || result.body == null) {" + "              init.body = result.body;"
                        ...
                        + "            if (result.credentials != null) {" + "              init.credentials = "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME + ".convertJsonToCredential(result.credentials);"
                        ...
                        + "          return fetch(resource, init);" + "        });" + "      });" + "    } else {"
                        + "      return fetch(resource, init);" + "    }" + "  };" + "})(window.fetch);";
}
