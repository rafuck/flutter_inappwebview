...
public class InterceptAjaxRequestJS {
        ...
        public static final String INTERCEPT_AJAX_REQUEST_JS_SOURCE = "(function(ajax) {"
                        ...
                        + "  var send = ajax.prototype.send;" + "  var open = ajax.prototype.open;"
                        ...
                        + "          callback(new Uint8Array(request.response));" + "          return;"
                        + "        case 'blob':" + "          const reader = new FileReader();"
                        + "          reader.addEventListener('loadend', function() {  "
                        + "            callback(new Uint8Array(reader.result));" + "          });"
                        + "          reader.readAsArrayBuffer(request.response.blob()); /*rafuck*/ "
                        + "          return;" + "        case 'document':"
                        + "          callback(request.response.documentElement.outerHTML);" + "          return;"
                        + "        case 'json':" + "          callback(request.response);" + "          return;"
                        + "      };" + "    }" + "    callback(null);" + "  };"
                        ...
                        + "    setRequestHeader.call(this, header, value);" + "  };" + "  function handleEvent(e) {"
                        + "    var self = this;"
                        ...
                        + FLAG_VARIABLE_FOR_SHOULD_INTERCEPT_AJAX_REQUEST_JS_SOURCE + " == true) {"
                        + "      var headers = this.getAllResponseHeaders();" + "      var responseHeaders = {};"
                        + "      if (headers != null) {" + "        var arr = headers.trim().split(/[\\r\\n]+/);"
                        + "        arr.forEach(function (line) {" + "          var parts = line.split(': ');"
                        + "          var header = parts.shift();" + "          var value = parts.join(': ');"
                        + "          responseHeaders[header] = value;" + "        });" + "      }"
                        + "      convertRequestResponse(this, function(response) {" + "        var ajaxRequest = {"
                        ...
                        + "          readyState: self.readyState," + "          status: self.status,"
                        + "          responseURL: self.responseURL," + "          responseType: self.responseType,"
                        + "          response: response,"
                        ...
                        + "          statusText: self.statusText," + "          responseHeaders, responseHeaders,"
                        + "          event: {" + "            type: e.type," + "            loaded: e.loaded,"
                        + "            lengthComputable: e.lengthComputable," + "            total: e.total"
                        + "          }" + "        };" + "        window." + JavaScriptBridgeJS.JAVASCRIPT_BRIDGE_NAME
                        + ".callHandler('onAjaxProgress', ajaxRequest).then(function(result) {"
                        + "          if (result != null) {" + "            switch (result) {" + "              case 0:"
                        + "                self.abort();" + "                return;" + "            };" + "          }"
                        ...
                        + "            var responseHeaders = {};" + "            if (headers != null) {"
                        ...
                        + "                responseType: self.responseType," + "                response: response,"
                        + "                responseText: (self.responseType == 'text' || self.responseType == '') ? self.responseText : null,"
                        + "                responseXML: (self.responseType == 'document' && self.responseXML != null) ? self.responseXML.documentElement.outerHTML : null,"
                        ...
                        + ".convertBodyRequest(data).then(function(data) {" + "        var ajaxRequest = {"
                        + "          data: data," + "          method: self._flutter_inappwebview_method,"
                        + "          url: self._flutter_inappwebview_url,"
                        + "          isAsync: self._flutter_inappwebview_isAsync,"
                        + "          user: self._flutter_inappwebview_user,"
                        + "          password: self._flutter_inappwebview_password,"
                        + "          withCredentials: self.withCredentials,"
                        + "          headers: self._flutter_inappwebview_request_headers,"
                        + "          responseType: self.responseType" + "        };" + "        window."
                        ...
                        + "              case 0:" + "                self.abort();" + "                return;"
                        + "            };" + "            if (result.data != null && !"
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.data) && result.data.length > 0) {" + "              var bodyString = "
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME + ".arrayBufferToString(result.data);"
                        ...
                        + "                }" + "              }" + "            }" + "            if ("
                        + JavaScriptBridgeJS.JAVASCRIPT_UTIL_VAR_NAME
                        + ".isString(result.data) || result.data == null) {" + "              data = result.data;"
